...
X = df_imputed.drop('final_result', axis=1)
y=df_imputed['final_result']
...
X_train, X_test, y_train, y_test = train_test_split(X,y_enc,test_size=0.3, random_state=42, stratify=y)
...
for name, model in models.items():
  ...
  for i in range(1,11):
    ...
    if f1>max_f1:
      ...
      best_selector = copy.deepcopy(selector)
      selectors[name] = best_selector
      max_accuracy=accuracy
      max_f1=f1
      max_train_time=train_time
      features_used=i

  results.append([name, best_model, best_selector, max_train_time, max_accuracy, max_f1, features_used])


results_df = pd.DataFrame(results, columns=['Model', 'Model_Object', 'Selector_Object', 'Train Time (s)', 'Accuracy', 'F1_Score', 'features_used'])
print(results_df)
...
for i, row in results_df.iterrows():
    ...
    f1 = round(f1_score(y_test, y_pred, average='weighted'), 3)
    print(f"F1 score for model {name}: {f1}")
...
joblib.dump(selectors,'selector.pkl')
